...
func TestIsMassdriverYAMLArtifactDefinition(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			path: "some/path/resource.yaml",
			want: false,
		},
		{
			name: "yml file",
			path: "some/path/resource.yml",
			want: false,
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := resourcetype.IsMassdriverYAMLResourceType(tc.path)
			if got != tc.want {
				t.Errorf("IsMassdriverYAMLResourceType(%q) = %v, want %v", tc.path, got, tc.want)
			}
		})
	}
}
...
func TestBuild(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			path:    filepath.Join("testdata", "massdriver-yaml-resource", "massdriver.yaml"),
			wantErr: false,
			check: func(t *testing.T, result map[string]any) {
				// Check $md block
				...
				if md["name"] != "test-resource" {
					t.Errorf("expected name to be test-resource, got %v", md["name"])
				}
				if md["label"] != "Test Resource Type" {
					t.Errorf("expected label to be Test Resource Type, got %v", md["label"])
				}
				...
				if result["title"] != "Test Resource" {
					t.Errorf("expected title to be Test Resource, got %v", result["title"])
				}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			result, err := resourcetype.Build(tc.path)
