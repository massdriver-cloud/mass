...
func (u *URLHelper) EnvironmentURL(projectID, environmentID string) string {
	return fmt.Sprintf("%s/orgs/%s/projects/%s/environments/%s", u.baseURL, u.orgID, projectID, environmentID)
}

// InstanceURL returns the URL for a specific package
func (u *URLHelper) InstanceURL(projectID, environmentID, instanceID string) string {
	return fmt.Sprintf("%s/orgs/%s/projects/%s/environments/%s?package=%s", u.baseURL, u.orgID, projectID, environmentID, instanceID)
}
